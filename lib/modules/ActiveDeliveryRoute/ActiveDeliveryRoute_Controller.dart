...
class ActiveDeliveryRouteController extends GetxController {
        ...
        onSubmit: (reason, reasonDetails) async {
          try {
            ...
            if (responseData != null && responseData['isSuccess'] == true) {
              // 3. تحديث الخطة بالبيانات الجديدة القادمة من الـ API
              final updatedDeliveryPlan = DeliveryPlan.fromJson(responseData['data']);
              plan.value = updatedDeliveryPlan;

              // 4. إعادة رسم الخريطة بالمسار والمسارات المحدثة
              await MapDrawerHelper.drawFullRoute(
                routeMapController: routeMapController,
                plan: plan.value!.toPlanResponse(),
              );

              // 5. تحديث مؤشر الزيارة النشطة وإعادة إنعاش الواجهة
  ...
  Future<void> updateVisitStatus({
    ...
    try {
      ...
      // 1. إرسال حالة الزيارة المحدثة للـ API
      ...
      if (response != null && response['isSuccess'] == true) {
        ...
        // 2. تحديث قائمة الزيارات بأسلوب copyWith النظيف
        if (plan.value != null) {
          ...
          // تحديث الخطة بالنسخة الجديدة
          plan.value = plan.value!.copyWith(visits: updatedVisits);

          // تحديث مؤشر الزيارة النشطة
  ...
  /// دالة جلب خطة اليوم وتحديث الـ State والرسم على الخريطة
  Future<void> fetchTodayDeliveryPlan() async {
    try {
      ...
      if (responseData != null && responseData['isSuccess'] == true && responseData['data'] != null) {
        // 1. تحويل الـ JSON إلى Model
        final updatedPlan = DeliveryPlan.fromJson(responseData['data']);
        plan.value = updatedPlan;

        // 2. إعادة رسم المسارات على الخريطة بالبيانات الجديدة
        await MapDrawerHelper.drawFullRoute(
          routeMapController: routeMapController,
          plan: plan.value!.toPlanResponse(),
        );

        // 3. تحديث مؤشر الزيارة النشطة للي بعده
        ...
        // 4. تحديث الـ Controllers وإعادة إنعاش الواجهة
        routeMapController.update();
        plan.refresh();
      }
    } catch (e) {
