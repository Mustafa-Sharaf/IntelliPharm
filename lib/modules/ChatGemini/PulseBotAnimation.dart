...
const List<Waypoint> WAYPOINTS = [
  Waypoint(
    dx: 0.0,
    dy: 0.0,
    holdRange: Duration(milliseconds: 1200),
    moveDurSeconds: 0.55,
  ),
  Waypoint(
    dx: -1.9,
    dy: 0.55,
    holdRange: Duration(milliseconds: 850),
    moveDurSeconds: 0.32,
  ),
  Waypoint(
    dx: -1.8,
    dy: 0.0,
    holdRange: Duration(milliseconds: 600),
    moveDurSeconds: 0.5,
  ),
  Waypoint(
    dx: -1.0,
    dy: -0.55,
    holdRange: Duration(milliseconds: 500),
    moveDurSeconds: 0.45,
  ),
  Waypoint(
    dx: 0.0,
    dy: -0.65,
    holdRange: Duration(milliseconds: 700),
    moveDurSeconds: 0.52,
  ),
  Waypoint(
    dx: 0.0,
    dy: 0.0,
    holdRange: Duration(milliseconds: 400),
    moveDurSeconds: 0.42,
  ),
  Waypoint(
    dx: 1.0,
...
class _BotExpressiveAnimationState extends State<BotExpressiveAnimation>
  ...
  void initState() {
    ...
    _blinkAnimation = TweenSequence<double>([
      ...
      TweenSequenceItem(
        ...
        ).chain(CurveTween(curve: const Cubic(0.34, 1.7, 0.64, 1))),
        weight: 70,
      ),
  ...
  void _scheduleGaze() {
    ...
    _gazeController.duration = Duration(
      milliseconds: (wp.moveDurSeconds * 1000).toInt(),
    );
    Curve curve = wp.moveDurSeconds <= 0.38
        ? const Cubic(0.34, 1.56, 0.64, 1)
        : const Cubic(0.25, 0.46, 0.45, 0.94);

    _gazeAnimation = Tween<Offset>(
      begin: _gazeAnimation.value,
      end: Offset(wp.dx, wp.dy),
    ).animate(CurvedAnimation(parent: _gazeController, curve: curve));
    _gazeController.forward(from: 0.0);

    _gazeTimer = Timer(
      Duration(
        milliseconds:
            wp.holdRange.inMilliseconds +
            (_random.nextDouble() * 100 - 50).toInt(),
      ),
      () {
        if (mounted) {
  ...
  void _scheduleBlink() {
    if (!mounted) return;
    _blinkTimer = Timer(
      Duration(milliseconds: (2400 + _random.nextDouble() * 2800).toInt()),
      () async {
        if (mounted) {
          ...
          if (_random.nextDouble() < 0.28) {
            _blinkTimer = Timer(
              Duration(
                milliseconds: (380 + _random.nextDouble() * 140).toInt(),
              ),
              () {
