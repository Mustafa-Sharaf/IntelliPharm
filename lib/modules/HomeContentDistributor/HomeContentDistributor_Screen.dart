...
class HomeContentDistributorScreen extends StatelessWidget {
  ...
  // ✅ الحقن الصحيح يكون خارج دالة build لضمان إنشائها مرة واحدة فقط
  //final DeliveryHomeController controller = Get.put(DeliveryHomeController());
  //final ActiveDeliveryRouteController deliveryController = Get.put(ActiveDeliveryRouteController());
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              // ✅ لا داعي لـ Obx ثانية هنا لأن الـ Obx الرئيسية تُراقب الصفحة بالكامل
              InkWell(
                onTap: deliveryController.isLoading.value
                    ? null
                    : () async {
                  bool isSuccess = await deliveryController.initiateDeliveryPlan();
                  if (isSuccess) {
              ...
              Row(
                ...
                children: [
                  ...
                  TextButton(
                    onPressed: () {
                    // Get.to(()=>ActiveDeliveryRouteScreen());
                      Get.toNamed('/activeDeliveryRoute');
                    },
...
class HomeContentDistributorScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 🟢 جلب الكنترولرات المحقونة تلقائياً
    final controller = Get.find<DeliveryHomeController>();
    final deliveryController = Get.find<ActiveDeliveryRouteController>();
            ...
            children: [
              ...
              InkWell(
                onTap: deliveryController.isLoading.value
                    ...
                    : () async {
                  bool isSuccess =
                  await deliveryController.initiateDeliveryPlan();
                  if (isSuccess) {
                    Get.toNamed('/activeDeliveryRoute');
                  }
                },
