...
class NewOrderController extends GetxController {
  ...
  //  إزالة notesController.dispose() لحمايته عند استخدام fenix: true

  void addToCart(MedicineModel med, int qty) {
    if (qty <= 0) {
      AppSnackBar.error("Quantity is 0".tr);
      return;
    }

    if (qty > med.availableQuantity) {
      AppSnackBar.error("Not enough stock".tr);
      return;
    }
  ...
  Future<void> submitOrder() async {
    ...
    try {
      ...
      if (pharmacy == null) {
        AppSnackBar.error("Please select pharmacy".tr);
        return;
      }

      if (cart.isEmpty) {
        AppSnackBar.error("Cart is empty".tr);
        return;
      }
      ...
      if (response["isSuccess"] == true) {
        // 🟢 تفريغ السلة والبيانات عند نجاح الطلب
        cart.clear();
        notesController.clear();
        pharmacyController.selectedPharmacy.value = null;

        AppSnackBar.success("Order created successfully".tr);

        // 3. العودة للشاشة السابقة بشكل آمن
       //

      }
    } catch (e) {
      AppSnackBar.error("Failed to create order".tr);
