...
class _VisitDetailsScreenState extends State<VisitDetailsScreen> {
  ...
  void initState() {
    super.initState();
    // 🟢 استخراج الأرجومينتس لمرة واحدة فقط في بداية دورة حياة الـ Widget بأمان
    final rawArgs = Get.arguments;
    if (rawArgs is Map) {
      pharmacyId = rawArgs["pharmacyId"] ?? 0;
      visitId = rawArgs["visitId"] ?? 0;
    } else {
      pharmacyId = 0;
      visitId = 0;
    }

    // 🟢 البحث عن الكنترولر وتثبيته في الـ State
    controller = Get.find<PharmacyDetailsController>(
      tag: pharmacyId.toString(),
    );
  }
  ...
  Widget build(BuildContext context) {
    final colors = Theme.of(context).extension<ThemeColors>()!;
    final size = MediaQuery.of(context).size;
    final visitDetailsController = Get.find<VisitDetailsController>();
